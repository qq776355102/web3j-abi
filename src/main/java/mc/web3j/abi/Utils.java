...
public class Utils {
    ...
    @SuppressWarnings("unchecked")
    public static <T> List<TypeReference<Type<T>>> convert(List<TypeReference<Type<T>>> input) {
        List<TypeReference<Type<T>>> result = new ArrayList<>(input.size());
        result.addAll(input.stream()
                .map(typeReference -> (TypeReference<Type<T>>) typeReference)
